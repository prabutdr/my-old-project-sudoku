...
public class Sudoku {
    ...
    public boolean setCellValue(int value, int row, int col) {
        ...
        if(row >= size || col >= size || row < 0 || col < 0 ||
                !table[row][col].isEmpty() || !table[row][col].isValidValue(value))
            return false;
        int i, j;
    ...
    public boolean redoLastAction() {
        Action action;
        while((action=actionStack.popRedoAction()) != null) {
            if(action.getCommand() == Action.FIX_VALUE) {
                actionStack.backupRevActionStack();
                this.setCellValue(action.getValue(), action.getRow(), action.getColumn());
                actionStack.restoreRevActionStack();
                return true;
            }
            else if(action.getCommand() == Action.REMOVE_CELL_VALUE) {
                actionStack.backupRevActionStack();
                this.removeCellValue(action.getRow(), action.getColumn());
                actionStack.restoreRevActionStack();
                return true;
            }
        }
    ...
    public int getHorizontalBoxCount() {
        return hDiv;
    }

    public int getVerticalBoxCount() {
        return vDiv;
    }
